/****************************************************************************
|*
|* nrt2xml: both directions XML<->TAP3 Converter.
|* 
...
<!--\n\
    Generated by: nrt2xml NRT-XML Converter\n\
                  copyright (c) 2005-2018 Javier Gutierrez\n\
                  All rights reserved\n\
                  tap3edit Tools (https://github.com/tap3edit)\n\
-->\n"
...
void    help                    ();
string  get_specfilename        (NrtInfo *nrtinfo);
...
int main(int argc, char **argv)
{
    ...
    if (getenv(TAP3XML_SPEC_VAR)!=NULL)     /* a. nrt2xml environment variable */
    ...
    if ( option == "-x" )
    {
        ...
        NrtInfo*    nrtinfo = new NrtInfo();
        nrtinfo->buffin(bfin_asn);
        ...
        spec_filename=get_specfilename(nrtinfo);
...
void help()
{
    cerr << "Copyright (C) 2005-2018 Javier Gutierrez <https://github.com/tap3edit>\n";
    cerr << "Usage: nrt2xml -x|-a inputfile [outputfile]\n";
    cerr << "  -x: ASN -> XML\n";
    cerr << "  -a: XML -> ASN\n";
    exit(1);
}
...
string get_specfilename(NrtInfo *nrtinfo)
{
    ...
    spec_filename << nrtinfo->type();
    spec_filename << setw(2) << setfill('0') << nrtinfo->version();
    spec_filename << setw(2) << setfill('0') << nrtinfo->release();
    spec_filename << ".asn";
